...
export default function Practice() {
  ...
  const renderOption = (option) => (
    <View key={option} style={{marginLeft:50}}>
      <View 
        style={[
          styles.choose, 
          faultOption === option && styles.fault,
  ...
  const [selectedOptions, setSelectedOptions] = useState(['B','C']);
  const [faultOptions, setFaultOptions] = useState(['A','C']);
  const moreOption = (option) => (
    <View key={option} style={{marginLeft:50}}>
      <View 
        style={[
          styles.choose, 
          faultOptions.includes(option) && styles.fault,
  ...
  return (
    <View style={{backgroundColor:'white',height:'100%',width:'100%'}}>
      ...
      <View>
        ...
        <View style={styles.sure}><Text style={{textAlign:'center',marginTop:10,color:'white'}}>下一题</Text></View>
      </View>
    </View>
  );
}
